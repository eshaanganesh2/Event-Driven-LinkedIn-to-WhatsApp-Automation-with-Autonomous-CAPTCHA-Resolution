...
I switched to Playwright for its handling of shadow DOMs and iframe-nested security elements, containerized the runtime to clear Lambda's 250MB deployment limit, and automated the full multi-stage challenge flow: checkbox CAPTCHA via Playwright's auto-waiting, audio challenges via pydub normalization and SpeechRecognition transcription, and PIN-based verification via a human-in-the-loop WhatsApp polling bridge backed by DynamoDB. Meta's requirement for a quick acknowlegement to any webhook request with a 200 HTTP status code response made a synchronous solution impossible (because of the cookie refresh flow) which drove the decoupled worker architecture at the core of this system. <br><br>
...
* **Intelligent Webhook Routing & Observability**: Implements a unified ingestion layer that parses Meta’s webhook JSON payloads to differentiate between user messages, interactive button replies, and delivery statuses. This ensures real-time logging of message delivery and facilitates immediate system feedback for both user-initiated commands and automated status updates.

---

## System Architecture
To meet Meta’s strict acknowledgement requirements, the webhook Handler immediately returns a 200 HTTP status code to prevent retrying and sending of duplicate payloads. All heavy lifting, including Playwright-based browser automation and DynamoDB operations, is offloaded to a Worker Lambda. This decoupling ensures that long-running cookie refresh flows never block the API response. <br><br>
...
To prevent Webhook timeouts, and subsequent duplicate cookie refresh retries, the API returns a `200 OK` immediately and invokes the **Worker Lambda** asynchronously.
...
A major architectural challenge was Meta's requirement for a quick acknowlegement to any webhook request with a 200 HTTP status code response. Because the cookie refresh flow involves heavy browser automation and human-in-the-loop MFA polling (taking 1–2 minutes), a synchronous response was impossible.
